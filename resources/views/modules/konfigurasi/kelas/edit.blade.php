        ...
        <div class="h-screen flex items-center justify-center relative -mt-[100px] -ml-[80px]">
            <div class="w-[1020px] h-[647px] absolute">
                <div class="w-[1020px] h-[647px] left-0 top-0 absolute bg-white rounded-[5px] shadow mx-[90px] px-[90px]">
                    {{-- FORM PRESENSI --}}
                    <form method="post"
                        action="{{ route('konfigurasi.kelas.update', ['id_kelas' => $dataKelas->id_kelas]) }}">
                        @csrf
                        {{-- <input type="hidden" name="_token" id="token" value="{{ csrf_token() }}"> --}}
                        {{-- FIELD NIS --}}
                        <input type="text" placeholder="Kelas" name="kelas" id="kelas" value="{{ $dataKelas->kelas }}"
                            ...
                            <li class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r">
                                <div class="flex items-center ps-3">
                                    <input type="radio" id="status_aktif" name="status" value="on" checked
                                        class="w-4 h-[62px] accent-gray-500 bg-gray-100 border-gray-300" {{ $dataKelas->status == 'on' ? 'checked' : '' }}>
                            ...
                            <li class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r">
                                <div class="flex items-center ps-3">
                                    <input type="radio" id="status_non_aktif" name="status" value="off"
                                        class="w-4 h-[62px] accent-gray-500 bg-gray-100 border-gray-300 " {{ $dataKelas->status == 'off' ? 'checked' : '' }}>
